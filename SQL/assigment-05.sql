...
HAVING COUNT(group_id) = 
	(SELECT MAX(cg)
	FROM 
		(SELECT COUNT(group_id) AS cg
		FROM group_account
		GROUP BY account_id) AS tkg);
-- Ở bài này mình cần thống kê 1 account gia nhập bao nhiêu group 
-- lấy ra người tham gia nhiều group nhất 
-- và thống kê các account tham gia số gr bằng người đấy
SELECT *
FROM account
JOIN (  
		SELECT account_id , COUNT(group_id) as cg
		FROM group_account
        GROUP BY account_id 
	 ) AS jmax USING (account_id)
WHERE jmax.cg = 
		(
        SELECT MAX(cg) 
        FROM (  
...
SELECT * , CHAR_LENGTH(content) - CHAR_LENGTH(REPLACE (content , ' ' , '')) + 1 AS numchar
FROM question 
...
HAVING COUNT(account_id) = 
	(SELECT MAX(ca)
    FROM 
		(SELECT COUNT(account_id) as ca
		FROM account
		GROUP BY department_id) AS tka);
