...
DROP TRIGGER IF EXISTS qs_01;
DELIMITER \\
CREATE TRIGGER qs_01
BEFORE INSERT ON `group`
FOR EACH ROW 
BEGIN
	IF NEW.created_date < CURRENT_DATE - INTERVAL 1 YEAR
    THEN 
...
INSERT INTO `group` (group_name , creator_id , created_date)
VALUES 				("MySQL"    , 1          , "2024-03-15");
-- Question 2: Tạo trigger Không cho phép người dùng thêm bất kỳ user nào vào
-- department "Sale" nữa, khi thêm thì hiện ra thông báo "Department "Sale" cannot add more user"
DROP TRIGGER IF EXISTS qs_02
DELIMITER \\
CREATE TRIGGER qs_02
BEFORE INSERT ON account
FOR EACH ROW
BEGIN 
	IF NEW.department_id = (SELECT department_id FROM department WHERE department_name = "Sale")
    THEN 
		SIGNAL SQLSTATE '45000'
...
DROP TRIGGER IF EXISTS qs_03;
DELIMITER \\
CREATE TRIGGER qs_03
BEFORE INSERT ON group_account
FOR EACH ROW
BEGIN
	IF NEW.group_id = ANY(SELECT group_id FROM group_account WHERE COUNT(account_id) >= 5 GROUP BY group_id)
    THEN 
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = "group da du thanh vien";
	END IF;
END \\
DELIMITER ;
...
DROP TRIGGER IF EXISTS qs_04;
DELIMITER \\
CREATE TRIGGER qs_04
...
BEGIN
    ...
    IF v_count_question > 10 
    THEN 
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Bai thi da co du cau hoi';
	END IF;
END \\
DELIMITER ;
...
DROP TRIGGER IF EXISTS qs_05;
DELIMITER \\
CREATE TRIGGER qs_05
...
BEGIN 
    ...
    THEN 
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'xoa tai khoang co email khong hop le';
	END IF;
END \\
DELIMITER ;
...
DROP TRIGGER IF EXISTS qs_06;
DELIMITER \\
CREATE TRIGGER qs_06
BEFORE INSERT ON account
FOR EACH ROW
BEGIN 
...
DROP TRIGGER IF EXISTS qs_07;
DELIMITER \\
CREATE TRIGGER qs_07
...
BEGIN
    ...
    THEN 
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'dap an dung dat toi da';
	END IF;
END \\
DELIMITER ;
...
DROP TRIGGER IF EXISTS qs_08;
DELIMITER \\
CREATE TRIGGER qs_08
BEFORE INSERT ON account
FOR EACH ROW
BEGIN 
	IF NEW.gender = 'nam' 
...
DROP TRIGGER IF EXISTS qs_10;
DELIMITER \\
CREATE TRIGGER qs_10
...
BEGIN
    ...
    THEN 
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Khong the xoa exam tao tu 2 ngay truoc';
	END IF;
END \\
DELIMITER ;
